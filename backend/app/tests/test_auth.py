...
def test_get_current_user_with_valid_firebase_token(
    mock_verify: MagicMock, client: TestClient
):
    ...
    response = client.get(
        "/api/v1/auth/me", headers={"Authorization": "Bearer valid_firebase_token"}
    )
...
@patch("app.core.auth.firebase_auth.verify_id_token")
def test_refresh_token_with_valid_token(mock_verify: MagicMock, client: TestClient):
    """Test token refresh with valid authentication"""
    mock_verify.return_value = {"uid": "test123", "email": "test@example.com"}

    response = client.post(
        "/api/v1/auth/refresh", headers={"Authorization": "Bearer valid_token"}
    )
